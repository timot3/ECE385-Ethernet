...
*   still compile but the program will not work. Saves about 60 uint8_ts SRAM and
*   1550 uint8_ts flash.
*/
...
* issued. Saves 4 uint8_ts SRAM and 550 uint8_t flash.
*/
...
*   requests. Saves 2 uint8_ts SRAM and 250 uint8_ts flash.
*/
...
*   about 40 uint8_ts SRAM and 200 uint8_ts flash. If building with -flto this does not
...
*   pings will not be called. Saves 2 uint8_ts SRAM and 230 uint8_ts flash.
*/
...
*   Saves 30 uint8_ts SRAM and 80 uint8_ts flash.
*/
...
public:
   ...
   *     @param  macaddr Hardware address to assign to the network interface (6 uint8_ts)
   ...
   *     @param  my_ip IP address (4 uint8_ts). 0 for no change.
   *     @param  gw_ip Gateway address (4 uint8_ts). 0 for no change. Default = 0
   *     @param  dns_ip DNS address (4 uint8_ts). 0 for no change. Default = 0
   *     @param  mask Subnet mask (4 uint8_ts). 0 for no change. Default = 0
   ...
   *     @param  plen Number of uint8_ts in packet
   ...
   *     @param  gwipaddr Gateway address (4 uint8_ts)
   */
   ...
   *     @param  dip Pointer to 4 uint8_t destination IP address
   ...
   *     @param  len Size of payload (maximum 220 octets / uint8_ts)
   *     @param  sport Source port
   *     @param  dip Pointer to 4 uint8_t destination IP address
   *     @param  dport Destination port
   */
   ...
   */
   ...
   /**   @brief  Send ping
   *     @param  destip Pointer to 4 uint8_t destination IP address
   */
   static void clientIcmpRequest (const uint8_t *destip);

   /**   @brief  Check for ping response
   *     @param  ip_monitoredhost Pointer to 4 uint8_t IP address of host to check
   ...
   *     @param  wolmac Pointer to 6 uint8_t hardware (MAC) address of host to send message to
   */
   ...
   *     @param  dst Pointer to the 4 uint8_t destination
   *     @param  src Pointer to the 4 uint8_t source
   *     @note   There is no check of source or destination size. Ensure both are 4 uint8_ts
   */
   static void copyIp (uint8_t *dst, const uint8_t *src);

   /**   @brief  Copies a hardware address
   *     @param  dst Pointer to the 6 uint8_t destination
   *     @param  src Pointer to the 6 uint8_t destination
   *     @note   There is no check of source or destination size. Ensure both are 6 uint8_ts
   */
   static void copyMac (uint8_t *dst, const uint8_t *src);

   /**   @brief  Output to serial port in dotted decimal IP format
   *     @param  buf Pointer to 4 uint8_t IP address
   *     @note   There is no check of source or destination size. Ensure both are 4 uint8_ts
   */
   ...
   */
   ...
   /**   @brief  Convert an IP address from dotted decimal formated string to 4 uint8_ts
   *     @param  uint8_tstr Pointer to the 4 uint8_t array to store IP address
   *     @param  str Pointer to string to parse
   *     @return <i>uint8_t</i> 0 on success
   */
   static uint8_t parseIp(uint8_t *uint8_tstr, const char *str);

   /**   @brief  Convert a uint8_t array to a human readable display string
   *     @param  resultstr Pointer to a buffer to hold the resulting null terminated string
   *     @param  uint8_tstr Pointer to the uint8_t array containing the address to convert
   ...
   */
   static void makeNetStr(char *resultstr,uint8_t *uint8_tstr,uint8_t len,
