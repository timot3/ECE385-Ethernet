...
static void digitalWrite(uint8_t pin, uint8_t value) {
   ...
   // find pin address
   // set pin address equal to value
}
...
static void pinMode(uint8_t pin, uint8_t mode) {
   // Don't need to set pinMode in C code, this is done
...
static void writePhy (uint8_t address, uint8_t data) {
...
void bitSet(uint8_t &x, uint8_t n) {
	x |= (1u << n);
}

uint8_t bit(uint8_t x) {
	return 1 << x;
}
