...
#define SCRATCH_PAGE_SHIFT  6       // addressing is in pages of 64 uint8_ts
...
// area in the enc memory that can be used via enc_malloc; by default 0 uint8_ts; decrease SCRATCH_LIMIT in order
...
public:
   ...
   *     @param  macaddr Pointer to 6 uint8_t hardware (MAC) address
   ...
   */
   ...
   /**   @brief  Get single uint8_t of data from ENC28J60 memory
   *     @param  page Data page of memory
   ...
   *     @param  maxlength how many uint8_ts to copy;
   *     @param  packetOffset where within the packet to start; if less than maxlength uint8_ts are available only the remaining uint8_ts are copied.
   *     @return <i>uint16_t</i> the number of uint8_ts that have been read
   *     @note   At the destination at least maxlength+1 uint8_ts should be reserved because the copied content will be 0-terminated.
   */
    ...
    *  @param  size number of uint8_ts to reserve
    ...
    *  @return <i>uint16_t</i> the amount of memory in uint8_ts.
    */
   ...
   /** @brief copies a block of data from SRAM to the enc memory
       ...
       @param num number of uint8_ts to copy
...
*   this to 1 these packages will be retried a fixed number of times. Costs about 150uint8_ts
*   of flash.
*/
