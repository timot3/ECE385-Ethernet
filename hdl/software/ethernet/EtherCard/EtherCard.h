...
/** Enable DHCP.
 *   Setting this to zero disables the use of DHCP; if a program uses DHCP it
 * will still compile but the program will not work. Saves about 60 bytes SRAM
 * and 1550 bytes flash.
 */
 ...
 *   about 40 bytes SRAM and 200 bytes flash. If building with -flto this does
 * not seem to save anything; maybe the linker is then smart enough to optimize
 * the call away.
 */
...
/** This type definition defines the structure of a UDP server event handler
 * callback function */
...
/** This type definition defines the structure of a DHCP Option callback
 * function */
...
public:
  ...
  static bool persist_tcp_connection; ///< False to break connections on first
                                      ///< packet received
  static uint16_t
      delaycnt; ///< Counts number of cycles of packetLoop when no packet
   ...
   *     @param  macaddr Hardware address to assign to the network interface (6
   * bytes)
   ...
   */
  static bool staticSetup(const uint8_t *my_ip, const uint8_t *gw_ip = 0,
                          const uint8_t *dns_ip = 0, const uint8_t *mask = 0);

  // tcpip.cpp
  /**   @brief  Sends a UDP packet to the IP address of last processed received
   * packet
   ...
   *     @param  plen Size of data to parse (e.g. return value of
   * packetReceive()).
   *     @return <i>uint16_t</i> Offset of TCP payload data in data buffer or
   * zero if packet processed
   ...
   *     @return <i>uint16_t</i> Offset within packet of TCP payload. Zero for
   * no data.
   */
  ...
  /**   @brief  Updates the broadcast address based on current IP address and
   * subnet mask
   */
   ...
   *     @param  datafill_cb Pointer to callback function that handles TCP data
   * payload
   *     @param  port Remote TCP/IP port to connect to
   *     @return <i>unit8_t</i> ID of TCP/IP session (0-7)
   *     @note   Return value provides id of the request to allow up to 7
   * concurrent requests
   */
  static uint8_t clientTcpReq(uint8_t (*result_cb)(uint8_t, uint8_t, uint16_t,
                                                   uint16_t),
   ...
   *     @param  additionalheaderline Pointer to c-string with additional HTTP
   * header info
   ...
   *     @param  dstport_l Destination port to expect response. Set to zero to
   * accept on any port
   ...
   *     @param  persist True to maintain TCP connection. False to finish TCP
   * connection after first packet.
   */
   ...
   */
  static bool
  udpServerHasProcessedPacket(uint16_t len); // called by tcpip, in packetLoop
   ...
   *     @param  fromRam Set true to indicate whether hname is in RAM or in
   * program space. Default = false
   *     @return <i>bool</i> True if DHCP successful
   *     @note   Blocks until DHCP complete or timeout after 60 seconds
   */
  static bool dhcpSetup(const char *hname = NULL, bool fromRam = true);
   ...
   */
  static void dhcpAddOptionCallback(uint8_t option,
                                    DhcpOptionCallback callback);

  /**   @brief  Register a callback for multiple DHCP option numbers
   *     @param  optionlist pointer to null terminate list of DHCP option
   * numbers (must be static)
   *     @param  callback The function to be call when the option is received
   */
  static void dhcpAddOptionCallback(uint8_t *optionlist,
                                    DhcpOptionCallback callback);

  // dns.cpp
  /**   @brief  Perform DNS lookup
   *     @param  name Host name to lookup
   *     @param  fromRam Set true to indicate whether name is in RAM or in
   * program space. Default = false
   ...
   *     @note   There is no check of source or destination size. Ensure both
   * are 4 bytes
   */
  ...
  /**   @brief  Output message and IP address to serial port in dotted decimal
   * IP format
  ...
  /**   @brief  Output Flash String Helper and IP address to serial port in
   ...
   */
  //    static void printIp (const __FlashStringHelper *ifsh, const uint8_t
  //    *buf);

  /**   @brief  Search for a string of the form key=value in a string that looks
   * like q?xyz=abc&uvw=defgh HTTP/1.1\\r\\n
   ...
   *     @param  key Pointer to null terminated string holding the key to search
   * for
   *     @return <i>unit_t</i> Length of the value. 0 if not found
   *     @note   Ensure strbuf has memory allocated of at least maxlen + 1 (to
   * accommodate result plus terminating null)
   */
  static uint8_t findKeyVal(const char *str, char *strbuf, uint8_t maxlen,
                            const char *key);

  /**   @brief  Decode a URL string e.g "hello%20joe" or "hello+joe" becomes
   * "hello joe"
   ...
   *     @param  urlbuf Pointer to a buffer to contain the null terminated
   * encoded URL
   *     @note   There must be enough space in urlbuf. In the worst case that is
   * 3 times the length of str
   */
  static void urlEncode(char *str, char *urlbuf);

  /**   @brief  Convert an IP address from dotted decimal formated string to 4
   * bytes
   ...
   *     @param  resultstr Pointer to a buffer to hold the resulting null
   * terminated string
   *     @param  bytestr Pointer to the byte array containing the address to
   * convert
   *     @param  len Length of the array (4 for IP address, 6 for hardware (MAC)
   * address)
   *     @param  separator Delimiter character (typically '.' for IP address and
   * ':' for hardware (MAC) address)
   *     @param  base Base for numerical representation (typically 10 for IP
   * address and 16 for hardware (MAC) address
   */
