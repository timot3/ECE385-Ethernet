 ...
 *******************************************************************************
 ...
 descriptor.  Then we check the VID and PID and make sure they appear in the
 TPL.
 */
...
//0x16 #define USB_ATTACHED_SUBSTATE_VALIDATE_VID_PID
//0x17 #define USB_ATTACHED_SUBSTATE_VALIDATE_CLSP                     0x18
...
//#define RUNNING_SUBSTATE_NORMAL_RUN
//0x0000  //
//#define RUNNING_SUBSTATE_SUSPEND_AND_RESUME
//0x0010  //
//#define RUNNING_SUBSUBSTATE_SUSPEND
//0x0000  //
//#define RUNNING_SUBSUBSTATE_RESUME
//0x0001  //
//#define RUNNING_SUBSUBSTATE_RESUME_WAIT
//0x0002  //
//#define RUNNING_SUBSUBSTATE_RESUME_RECOVERY
//0x0003  //
//#define RUNNING_SUBSUBSTATE_RESUME_RECOVERY_WAIT			0x0004
////
//#define RUNNING_SUBSUBSTATE_RESUME_COMPLETE
//0x0005  //
...
////******************************************************************************
//
//#define _USB_InitErrorCounters()        { numCommandTries   =
//USB_NUM_COMMAND_TRIES; } #define _USB_SetDATA01(x)               {
//pCurrentEndpoint->status.bfNextDATA01 = x; } #define _USB_SetErrorCode(x) {
//usbDeviceInfo.errorCode = x; } #define _USB_SetHoldState()             {
//usbHostState = STATE_HOLDING; } #define _USB_SetNextState()             {
//usbHostState = (usbHostState & STATE_MASK) + NEXT_STATE; } #define
//_host_tasks_SetNextSubState()   { host_tasks_state =( host_tasks_state &
//(STATE_MASK | SUBSTATE_MASK)) + NEXT_SUBSTATE; } #define
//_USB_SetNextSubSubState()       { usbHostState =  usbHostState +
//NEXT_SUBSUBSTATE; } #define _USB_SetNextTransferState()     {
//pCurrentEndpoint->transferState ++; } #define _USB_SetPreviousSubSubState() {
//usbHostState =  usbHostState - NEXT_SUBSUBSTATE; } #define
//_USB_SetTransferErrorState(x)   { x->transferState = (x->transferState &
//TSTATE_MASK) | TSUBSTATE_ERROR; } #define freez(x)                        {
//free(x); x = NULL; }
...
/* USB Mass Storage Device Information
 ...
 This structure is used to hold all the information about an attached Mass
 Storage device.
 */
typedef struct _USB_MSD_DEVICE_INFO {
  ...
  DWORD bytesTransferred; // Number of bytes transferred to/from the user's data
                          // buffer.
...
// typedef BOOL (* rom CLASS_EVENT_HANDLER) ( BYTE address, BYTE event, void
// *data, DWORD size );
//
