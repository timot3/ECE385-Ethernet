...
#define USB_NUMTARGETS                                                         \
  4 // number of targets in TPL, not counting uninitialized device
...
#define USB_REQUEST_GET_CONFIGURATION                                          \
  8 // Standard Device Request - GET CONFIGURATION
...
#define USB_FEATURE_DEVICE_REMOTE_WAKEUP                                       \
  1                             // CLEAR/SET FEATURE - Device remote wake-up
#define USB_FEATURE_TEST_MODE 2 // CLEAR/SET FEATURE - Test mode

/* Setup Data Constants */

#define USB_SETUP_HOST_TO_DEVICE                                               \
  0x00 // Device Request bmRequestType transfer direction - host to device
       // transfer
#define USB_SETUP_DEVICE_TO_HOST                                               \
  0x80 // Device Request bmRequestType transfer direction - device to host
       // transfer
#define USB_SETUP_TYPE_STANDARD                                                \
  0x00 // Device Request bmRequestType type - standard
#define USB_SETUP_TYPE_CLASS 0x20  // Device Request bmRequestType type - class
#define USB_SETUP_TYPE_VENDOR 0x40 // Device Request bmRequestType type - vendor
#define USB_SETUP_RECIPIENT_DEVICE                                             \
  0x00 // Device Request bmRequestType recipient - device
#define USB_SETUP_RECIPIENT_INTERFACE                                          \
  0x01 // Device Request bmRequestType recipient - interface
#define USB_SETUP_RECIPIENT_ENDPOINT                                           \
  0x02 // Device Request bmRequestType recipient - endpoint
#define USB_SETUP_RECIPIENT_OTHER                                              \
  0x03 // Device Request bmRequestType recipient - other

/* USB descriptors  */

#define USB_DESCRIPTOR_DEVICE 0x01 // bDescriptorType for a Device Descriptor.
#define USB_DESCRIPTOR_CONFIGURATION                                           \
  0x02 // bDescriptorType for a Configuration Descriptor.
#define USB_DESCRIPTOR_STRING 0x03 // bDescriptorType for a String Descriptor.
#define USB_DESCRIPTOR_INTERFACE                                               \
  0x04 // bDescriptorType for an Interface Descriptor.
#define USB_DESCRIPTOR_ENDPOINT                                                \
  0x05 // bDescriptorType for an Endpoint Descriptor.
#define USB_DESCRIPTOR_DEVICE_QUALIFIER                                        \
  0x06 // bDescriptorType for a Device Qualifier.
#define USB_DESCRIPTOR_OTHER_SPEED                                             \
  0x07 // bDescriptorType for a Other Speed Configuration.
#define USB_DESCRIPTOR_INTERFACE_POWER                                         \
  0x08                          // bDescriptorType for Interface Power.
#define USB_DESCRIPTOR_OTG 0x09 // bDescriptorType for an OTG Descriptor.

/* OTG SET FEATURE Constants    */
#define OTG_FEATURE_B_HNP_ENABLE                                               \
  3 // SET FEATURE OTG - Enable B device to perform HNP
#define OTG_FEATURE_A_HNP_SUPPORT 4 // SET FEATURE OTG - A device supports HNP
#define OTG_FEATURE_A_ALT_HNP_SUPPORT                                          \
  5 // SET FEATURE OTG - Another port on the A device supports HNP

/* USB Endpoint Transfer Types  */
#define USB_TRANSFER_TYPE_CONTROL 0x00 // Endpoint is a control endpoint.
#define USB_TRANSFER_TYPE_ISOCHRONOUS                                          \
  0x01                                   // Endpoint is an isochronous endpoint.
#define USB_TRANSFER_TYPE_BULK 0x02      // Endpoint is a bulk endpoint.
#define USB_TRANSFER_TYPE_INTERRUPT 0x03 // Endpoint is an interrupt endpoint.
#define bmUSB_TRANSFER_TYPE                                                    \
  0x03 // bit mask to separate transfer type from ISO attributes
...
typedef struct {
  BYTE epAddr; // copy from endpoint descriptor. Bit 7 indicates direction (
...
// NOTE: this is currently not implemented - typically an embedded host will
// provide a TPL to enumerate supported devices.
typedef struct {
  union {
    DWORD val;
    struct {
...
#define bmREQ_SET                                                              \
  USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_STANDARD |                         \
      USB_SETUP_RECIPIENT_DEVICE // set request type for all but 'set feature'
                                 // and 'set interface'
...
// char XferCtrlReq( BYTE addr, BYTE ep, BYTE bmReqType, BYTE bRequest, BYTE
// wValLo, BYTE wValHi, WORD wInd, WORD nbytes, char* dataptr )
...
#define XferSetAddr(oldaddr, ep, newaddr)                                      \
  XferCtrlReq(oldaddr, ep, bmREQ_SET, USB_REQUEST_SET_ADDRESS, newaddr, 0x00,  \
              0x0000, 0x0000, NULL)
/* Set Configuration Request  */
#define XferSetConf(addr, ep, conf_value)                                      \
  XferCtrlReq(addr, ep, bmREQ_SET, USB_REQUEST_SET_CONFIGURATION, conf_value,  \
              0x00, 0x0000, 0x0000, NULL)
///* Get configuration request  */
//#define bXferGetConf( addr, ep, urb_ptr ) bXferCtrlReq( addr, ep, 1, (
//bmREQ_GET_DESCR ), USB_REQUEST_GET_CONFIGURATION, 0x00, 0x00, 0x00, urb_ptr );
/* Get device descriptor request macro */
#define XferGetDevDescr(addr, ep, nbytes, dataptr)                             \
  XferCtrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, 0x00,     \
              USB_DESCRIPTOR_DEVICE, 0x0000, nbytes, dataptr)
///* Get configuration descriptor request macro */
#define XferGetConfDescr(addr, ep, nbytes, conf, dataptr)                      \
  XferCtrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, conf,     \
              USB_DESCRIPTOR_CONFIGURATION, 0x0000, nbytes, dataptr)
///* Get string descriptor request macro    */
#define XferGetStrDescr(addr, ep, nbytes, index, langid, dataptr)              \
  XferCtrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, index,    \
              USB_DESCRIPTOR_STRING, langid, nbytes, dataptr)
///* Get MAX LUN MSD class request macro */
//#define bXferGetMaxLUN( addr, intf, urb_ptr ) bXferCtrlReq( addr, 0, 1, (
//bmREQ_CL_GET_INTF ), USB_MSD_GET_MAX_LUN, 0, 0, intf, urb_ptr )
/* class requests */
#define XferSetProto(addr, ep, interface, protocol)                            \
  XferCtrlReq(addr, ep, bmREQ_HIDOUT, HID_REQUEST_SET_PROTOCOL, protocol,      \
              0x00, interface, 0x0000, NULL)
#define XferGetProto(addr, ep, interface, dataptr)                             \
  XferCtrlReq(addr, ep, bmREQ_HIDIN, HID_REQUEST_GET_PROTOCOL, 0x00, 0x00,     \
              interface, 0x0001, dataptr)
#define XferGetIdle(addr, ep, interface, reportID, dataptr)                    \
  XferCtrlReq(addr, ep, bmREQ_HIDIN, HID_REQUEST_GET_IDLE, reportID, 0,        \
              interface, 0x0001, dataptr)
...
// BYTE startCtrlReq( BYTE addr, BYTE ep, BYTE bmReqType, BYTE bRequest, BYTE
// wValLo, BYTE wValHi, WORD wInd, WORD nbytes, char* dataptr );
