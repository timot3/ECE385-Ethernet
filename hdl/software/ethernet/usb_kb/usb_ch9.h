/*
 ...
 In addition to that, class-specific descriptors are typedef'd here as well to
 keep descriptors together. They are typedefs anyway and won't take any real
 code space.
 */
...
typedef struct {
  ...
  BYTE bDeviceClass;    // Class code (assigned by the USB-IF). 0xFF-Vendor
                        // specific.
  BYTE bDeviceSubClass; // Subclass code (assigned by the USB-IF).
  BYTE bDeviceProtocol; // Protocol code (assigned by the USB-IF). 0xFF-Vendor
                        // specific.
  ...
  BYTE iSerialNumber; // Index of String Descriptor with the device's serial
                      // number.
...
//powered. #define USB_CFG_DSC_REM_WAKE    (0x20|USB_CFG_DSC_REQUIRED) // Device
//can request remote wakup
...
typedef struct {
  ...
  BYTE bInterfaceClass;    // Class code (assigned by the USB-IF).  0xFF-Vendor
                           // specific.
  BYTE bInterfaceSubClass; // Subclass code (assigned by the USB-IF).
  BYTE bInterfaceProtocol; // Protocol code (assigned by the USB-IF).
...
typedef struct {
  ...
  BYTE bEndpointAddress; // Endpoint address. Bit 7 indicates direction (0=OUT,
                         // 1=IN).
...
#define PID_MASK_DATA_SHIFTED                                                  \
  (PID_MASK_DATA << 2) // Data PID shift to proper position

#define LANG_EN_US                                                             \
  0x0409 // US language code, probably the only supported by string descriptors
