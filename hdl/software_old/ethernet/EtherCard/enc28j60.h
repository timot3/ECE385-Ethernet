...
#define RXSTART_INIT                                                           \
  0x0000 // start of RX buffer, (must be zero, Rev. B4 Errata point 5)
...
#define SCRATCH_MAP_SIZE                                                       \
  (((SCRATCH_PAGE_NUM % 8) == 0) ? (SCRATCH_PAGE_NUM / 8)                      \
                                 : (SCRATCH_PAGE_NUM / 8 + 1))

// area in the enc memory that can be used via enc_malloc; by default 0 bytes;
// decrease SCRATCH_LIMIT in order to use this functionality
#define ENC_HEAP_START SCRATCH_LIMIT
#define ENC_HEAP_END 0x2000

/** This class provide low-level interfacing with the ENC28J60 network
 * interface. This is used by the EtherCard class and not intended for use by
 * (normal) end users. */
class ENC28J60 {
public:
  static uint8_t buffer[];    //!< Data buffer (shared by receive and transmit)
  static uint16_t bufferSize; //!< Size of data buffer
  static bool broadcast_enabled; //!< True if broadcasts enabled (used to allow
                                 //!< temporary disable of broadcast for DHCP or
   ...
   *     @param  csPin Arduino pin used for chip select (enable network
   * interface SPI bus). Default = 8
   ...
   *     @note   All messages will be accepted, even messages with destination
   * MAC other than own
   ...
   *     @param  temporary Set true to only disable if temporarily enabled
   *     @note   This will reduce load on received data handling
   *     @note   In this mode only unicast and broadcast messages will be
   * received
   */
   ...
   *     @param  packetOffset where within the packet to start; if less than
   * maxlength bytes are available only the remaining bytes are copied.
   *     @return <i>uint16_t</i> the number of bytes that have been read
   *     @note   At the destination at least maxlength+1 bytes should be
   * reserved because the copied content will be 0-terminated.
   */
  static uint16_t readPacketSlice(char *dest, int16_t maxlength,
                                  int16_t packetOffset);

  /** @brief  reserves a block of RAM in the memory of the enc chip
   *  @param  size number of bytes to reserve
   *  @return <i>uint16_t</i> start address of the block within the enc memory.
   * 0 if the remaining memory for malloc operation is less than size.
   *  @note  There is no enc_free(), i.e., reserved blocks stay reserved for the
   * duration of the program.
   *  @note  The total memory available for malloc-operations is determined by
   * ENC_HEAP_END-ENC_HEAP_START, defined in enc28j60.h; by default this is 0,
   * i.e., you have to change these values in order to use enc_malloc().
   */
  static uint16_t enc_malloc(uint16_t size);

  /** @brief  returns the amount of memory within the enc28j60 chip that is
   * still available for malloc.
 ...
 *   The transmission hardware may drop some packets because it thinks a late
 * collision occurred (which should never happen if all cable length etc. are
 * ok). If setting this to 1 these packages will be retried a fixed number of
 * times. Costs about 150bytes of flash.
 */
#define ETHERCARD_RETRY_LATECOLLISIONS 0

/** Enable pipelining of packet transmissions.
 *   If enabled the packetSend function will not block/wait until the packet is
 * actually transmitted; but instead this wait is shifted to the next time that
 * packetSend is called. This gives higher performance; however in combination
 * with ETHERCARD_RETRY_LATECOLLISIONS this may lead to problems because a
 * packet whose transmission fails because the ENC-chip thinks that it is a late
 * collision will not be retried until the next call to packetSend.
 */
