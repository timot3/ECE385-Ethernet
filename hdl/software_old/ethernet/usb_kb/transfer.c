...
EP_RECORD dev0ep = {
    {0}}; // Endpoint data structure for uninitialized device during enumeration
EP_RECORD msd_ep[3] = {{0}}; // Mass storage bulk-only transport endpoints: 1
                             // control and 2 bulk, IN and OUT
...
//{ INIT_VID_PID( 0781, 5406 ), 0, 3, msd_ep, MSD_DRIVER, "Mass storage"  },
////Sandisk U3 Cruzer Micro
////{ INIT_VID_PID( 0CF2, 6220 ), 0, 0 }, //ENE UB6220
//{ INIT_CL_SC_P( 03, 01, 02 ), 0, 3, hid_ep, HIDM_DRIVER, "HID Mouse with Boot
//protocol" },	                // { INIT_VID_PID( aaaa, 5555 ), 0, 1, NULL, 0,
//NULL },	                // { INIT_CL_SC_P( 08, 06, 50 ), 0, 3, msd_ep,
//MSD_DRIVER, "Mass storage" }	    //Mass storage bulk only class
//};

/* control transfers function pointers */
const CTRL_XFER ctrl_xfers[2] = {XferCtrlND, XferCtrlData};
...
CLASS_CALLBACK_TABLE ClientDriverTable[USB_NUMCLASSES] = {
    ...
    {HIDMProbe, // HID class device init
     HIDMEventHandler, 0},
    {HIDKProbe, HIDKEventHandler, 0},
    {DummyProbe, DummyEventHandler, 0}};
...
/* Control transfer. Sets address, endpoint, fills control packet with necessary
 * data, dispatches control packet, and initiates bulk IN transfer,   */
...
/* Dispatch a packet. Assumes peripheral address is set and, if necessary,
...
/* IN transfer to arbitrary endpoint. Assumes PERADDR is set. Handles multiple
